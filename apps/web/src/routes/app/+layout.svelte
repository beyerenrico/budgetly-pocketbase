<script>
	import { BookOpen, Home, Icon } from 'svelte-hero-icons';
...
<div class="flex h-full flex-col">
	<!-- Top nav-->
	<header class="relative flex h-12 flex-shrink-0 items-center bg-white">
		<!-- Logo area -->
	...
	<div class="flex min-h-0 flex-1 overflow-hidden">
		<!-- Narrow sidebar-->
		<nav aria-label="Sidebar" class="hidden md:block md:flex-shrink-0 md:bg-gray-800">
			{#each sidebarItems as element}
				<div class="relative flex w-16 flex-col space-y-3 p-2">
					<a
						href={element.href}
						class="bg-gray-900 text-white flex-shrink-0 inline-flex items-center justify-center h-12 w-12 rounded-lg tooltip tooltip-right"
						data-tip={element.title}
					>
						<span class="sr-only">Open</span>
						<Icon src={element.icon} outline class="h-6 w-6" />
					</a>
				</div>
			{/each}
		</nav>
